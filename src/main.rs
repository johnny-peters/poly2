...
use poly2::{
    ...
    ExecutionReport, ExecutionSettings,
...
pub(crate) async fn fetch_clob_best_bid(token_id: &str) -> anyhow::Result<Decimal> {
...
#[derive(Clone)]
pub(crate) struct WsPriceCache {
    pub(crate) up: Arc<RwLock<TokenPrices>>,
    pub(crate) down: Arc<RwLock<TokenPrices>>,
    pub(crate) up_token_id: String,
    pub(crate) down_token_id: String,
}

impl WsPriceCache {
    pub(crate) fn new(up_token_id: &str, down_token_id: &str) -> Self {
        Self {
    ...
    pub(crate) async fn up_ask(&self) -> Decimal {
        self.up.read().await.best_ask
    }
    pub(crate) async fn down_ask(&self) -> Decimal {
        self.down.read().await.best_ask
    }
    pub(crate) async fn bid_for(&self, token_id: &str) -> Decimal {
    ...
    pub(crate) async fn ask_for(&self, token_id: &str) -> Decimal {
        if token_id == self.up_token_id {
            self.up.read().await.best_ask
        } else {
            self.down.read().await.best_ask
...
pub(crate) fn spawn_ws_price_feed(cache: WsPriceCache, cancel: tokio::sync::watch::Receiver<bool>) -> tokio::task::JoinHandle<()> {
            ...
            loop {
                ...
                match event {
                    "book" => {
                        ...
                        // asks sorted ascending (lowest first) 鈫?first() = best ask
                        ...
                        // bids sorted descending (highest first) 鈫?first() = best bid
...
pub(crate) fn read_decimal_env_or(dotenv_path: &Path, key: &str, default_value: Decimal) -> Decimal {
    resolve_runtime_var(key, dotenv_path)
        .and_then(|v| Decimal::from_str(v.trim()).ok())
        .unwrap_or(default_value)
}

pub(crate) fn read_u64_env_or(dotenv_path: &Path, key: &str, default_value: u64) -> u64 {
    resolve_runtime_var(key, dotenv_path)
...
pub(crate) fn append_round_record(
    log_path: &Path,
    round: u64,
    market_slug: &str,
    side: String,
...
pub(crate) fn print_execution_report(report: &ExecutionReport) {
    println!(
...
pub(crate) fn resolve_runtime_var(key: &str, dotenv_path: &Path) -> Option<String> {
    std::env::var(key)
        .ok()
